...
export const addLabel = createAsyncThunk(
  ...
  async (newLabel, { dispatch }) => {  
    ...
    const response = await fetch('https://www.laabamone.com/appoint_api.php?eventtype=teamadd&title='+newLabel.title+'&color='+encodeURIComponent(newLabel.color));
    const json =  await response.json();
   // console.log(json[0]);  
    return json[0];
...
export const updateLabel = createAsyncThunk(
  ...
  async (label, { dispatch }) => {
    ...
    const response = await fetch('https://www.laabamone.com/appoint_api.php?eventtype=teamadd&id='+label.id+'&title='+label.title+'&color='+encodeURIComponent(label.color));
    const json =  await response.json();
   // console.log(json[0]);  
    return json[0];
